...
def inspect_object(par, obj, objinfo, lamlines_found, ston_found, g102zeros, g141zeros, linelistoutfile, commentsfile, show_dispersed=True):
    ...
    while (done == 0):
        ...
        snr_meas_array = np.array([fitresults['oii_flux']/fitresults['oii_error'],
                fitresults['hg_flux']/fitresults['hg_error'],
                fitresults['hb_flux']/fitresults['hb_error'],
                fitresults['oiii_flux']/fitresults['oiii_error'],
                fitresults['hanii_flux']/fitresults['hanii_error'],
                fitresults['sii_flux']/fitresults['sii_error'],
                fitresults['siii_9069_flux']/fitresults['siii_9069_error'],
                fitresults['siii_9532_flux']/fitresults['siii_9532_error'],
                fitresults['he1_flux']/fitresults['he1_error']])

        # plot the whole goddamn thing
        plot_object(zguess, spdata, config_pars, snr_meas_array, full_fitmodel, full_contmodel, lamlines_found, index_of_strongest_line, contmodel, plottitle)
    ...
    if zset == 1:
        ...
        writeToCatalog(linelistoutfile, par, obj, ra, dec, a_image, b_image, jmag, hmag, fitresults, flagcont)


        writeFitdata(fitdatafilename, spec_lam, spec_val, spec_unc, spec_con, spec_zer, full_fitmodel, full_contmodel, mask_flg)

        fitspec_pickle = open(fitdatafilename + '.pickle', 'wb')
        output_meta_data = [par, obj, ra, dec, a_image, b_image, jmag, hmag, fitresults, flagcont, config_pars]
        pickle.dump(output_meta_data, fitspec_pickle)
        fitspec_pickle.close()

    # write comments to file
    # if we go back to the previous objects, duplicate comments will still be written
